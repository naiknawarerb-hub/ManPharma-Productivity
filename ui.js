(function () {
  ...
  function renderTaskCategories() {
    const wrap = q("#taskCategoryChips");
    ...
    if (formWrap) {
      formWrap.innerHTML = list.map((x) => `<button type="button" class="chip ${x.active}" data-value="${escapeHtml(x.value)}">${escapeHtml(x.label)}</button>`).join("");
    }
  ...
  function daySectionHtml(items, emptyText) {
    ...
    return sections
      .map((section) => {
        ...
        if (!rows.length) return `<div class="scheduled-cell"><p class="meta-line"><strong>${section.label}</strong></p><p class="meta-line">${emptyText}</p></div>`;
        return `<div class="scheduled-cell"><p class="meta-line"><strong>${section.label}</strong></p>${rows.map((r) => `<p class="meta-line">${escapeHtml(r.title)}</p>`).join("")}</div>`;
      })
      .join("");
  }
  ...
  function renderDashboardDateViews(items) {
    ...
    if (dayList) {
      dayList.innerHTML = dayItems.length
        ? `<div class="scheduled-grid">${daySectionHtml(dayItems, "No items")}</div>`
        : `<p class="meta-line">${viewState.search ? "No results found." : "No items for selected date."}</p>`;
    }
    ...
    weekList.innerHTML = days
      .map((dateKey) => {
        ...
        return `
          <details class="scheduled-group">
            <summary class="scheduled-head">
              <strong>${escapeHtml(formatDateOnly(dateKey))}</strong>
              <div class="inline-mini-actions">
                <button type="button" class="mini-btn mini-task" data-action="add-for-date" data-kind="task" data-date="${dateKey}">➕ Task</button>
                <button type="button" class="mini-btn mini-content" data-action="add-for-date" data-kind="post" data-date="${dateKey}">➕ Content</button>
                <button type="button" class="mini-btn mini-revenue" data-action="add-for-date" data-kind="revenue" data-date="${dateKey}">➕ Revenue</button>
              </div>
            </summary>
