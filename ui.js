(function () {
  ...
  function renderNotifications(items) {
    ...
    items.filter((i) => i.module === "task" && !i.completed).forEach((task) => {
      if (isDueNowOrPast(task.dueDate)) notifications.push(`Task due: ${task.title} (${formatDate(task.dueDate)})`);
    });

    items.filter((i) => i.module === "post" && !i.completed).forEach((post) => {
      if (isDueNowOrPast(post.dueDate)) notifications.push(`Post scheduled: ${post.title} (${formatDate(post.dueDate)})`);
    });
