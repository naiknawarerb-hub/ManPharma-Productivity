(function () {
  ...
  function bindSmartFilterBar() {
    const dashboard = q("#dashboardView");
    if (!dashboard) return;

    dashboard.addEventListener("scroll", () => {
      const current = dashboard.scrollTop;
      if (current <= 72) {
        setFilterBarVisibility(true);
      } else if (current > filterLastScroll && current > 120) {
        setFilterBarVisibility(false);
      } else if (current < filterLastScroll) {
        setFilterBarVisibility(true);
      }
      filterLastScroll = current;
    }, { passive: true });
  }
    ...
    qa(".nav-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (id !== "dashboardView") {
          ...
          const dashboard = q("#dashboardView");
          filterLastScroll = Number(dashboard?.scrollTop || 0);
          setFilterBarVisibility(filterLastScroll <= 120);
        }
      });
    });
