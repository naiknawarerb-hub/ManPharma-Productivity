(function () {
  ...
  function dueNotifications(items) {
    ...
    items.filter((i) => i.module === "task" && !i.completed).forEach((task) => {
      if (isDueNowOrPast(task.dueDate)) alerts.push(`Task due: ${task.title} (${formatDate(task.dueDate)})`);
    });

    items.filter((i) => i.module === "post" && !i.completed).forEach((post) => {
      if (isDueNowOrPast(post.dueDate)) alerts.push(`Post scheduled: ${post.title} (${formatDate(post.dueDate)})`);
    });
