(function () {
  ...
  function baseItem(partial) {
    ...
    return {
      ...
      meta: {
        ...(item.meta || {}),
        attachments: Array.isArray(item.meta?.attachments) ? item.meta.attachments : []
      }
    };
  }
  ...
  function openAttachmentDb() {
    return new Promise((resolve, reject) => {
      ...
      const req = window.indexedDB.open(ATTACHMENT_DB, 1);
      req.onupgradeneeded = () => {
        const db = req.result;
  ...
  function loadFallbackFiles() {
    return safeJsonParse(localStorage.getItem(FILES_KEY) || "{}", {});
  }

  function saveFallbackFiles(map) {
  ...
  async function saveAttachment(file) {
    ...
    try {
      ...
      const map = loadFallbackFiles();
      map[id] = record;
      saveFallbackFiles(map);
    }
  ...
  async function saveAttachmentsFromInput(inputEl) {
    const files = Array.from(inputEl?.files || []);
    const saved = [];
    for (const file of files) {
      // Prevent localStorage/IndexedDB overload in offline PWA.
      if (Number(file.size || 0) > 10 * 1024 * 1024) continue;
      const meta = await saveAttachment(file);
      if (meta) saved.push(meta);
    }
    return saved;
  }
